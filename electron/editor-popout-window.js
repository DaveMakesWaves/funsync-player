...
function open(parent, store) {
  ...
  popoutWindow.on('closed', () => {
    ...
    if (parent && !parent.isDestroyed()) {
      ...
      parent.webContents.send('editor-popout:closed');
    }
  });

  if (parent && !parent.isDestroyed()) {
    parent.webContents.send('editor-popout:opened');
  }
...
function relay(direction, payload) {
  if (direction === 'to-popout' && isOpen()) {
    popoutWindow.webContents.send('editor-popout:message', payload);
  } else if (direction === 'to-parent' && parentWindow && !parentWindow.isDestroyed()) {
    parentWindow.webContents.send('editor-popout:message', payload);
  }
}
