...
// Populate header + search chrome with inline SVGs (Lucide set, currentColor).
// Single-append at startup — syncControlsUI() later swaps the view-toggle icon.
backBtn.appendChild(svgIcon('arrowLeft', 20));
refreshBtn.appendChild(svgIcon('refresh', 16));
searchClear.appendChild(svgIcon('x', 14));
searchIconSlot.appendChild(svgIcon('search', 16));
folderToggle.appendChild(svgIcon('folder', 16));
...
async function renderList() {
  ...
  filtersRowEl.classList.remove('controls__row--hidden');
  filtersRowEl.classList.remove('controls__row--folder-root');
  syncControlsUI();
...
function applyFilters(videos) {
  ...
  if (uiState.search) out = fuzzySearch(out, uiState.search);
...
async function renderGroupingsList(kind) {
  ...
  // only to the videos inside. Hide the filter row in the list view.
  filtersRowEl.classList.add('controls__row--hidden');
  ...
  // Client-side filter by search query (name only for groupings)
  let filtered = items;
  if (uiState.search) {
...
async function renderFolderRoot() {
  controlsEl.classList.remove('controls--hidden');
  // Show the filter row so Matched/VR filters can hide empty sources. We
  // can't show sort (sources sort alphabetically), so the row is pared
  // down to tabs + VR filter — the `folder-root` class on the row lets
  // CSS hide the sort dropdown.
  filtersRowEl.classList.remove('controls__row--hidden');
  filtersRowEl.classList.remove('controls__row--folder-root');
  filtersRowEl.classList.add('controls__row--folder-root');
...
async function renderFolderAt(canonicalFolderPath) {
  ...
  // Breadcrumb trail above the grid.
  ...
  // Leading "All sources" that returns to the folder root.
...
vrSelect.addEventListener('change', () => {
  uiState.vr = vrSelect.value;
  saveFilterState();
  renderListIfActive();
});
for (const tab of document.querySelectorAll('.controls__tab')) {
...
// regular library list; the hash change re-runs `render()`.
...
// playlists / categories). Hash navigation so the phone's back button works.
for (const btn of modeTabsEl.querySelectorAll('.controls__mode')) {
  btn.addEventListener('click', () => {
    const mode = btn.dataset.mode;
    // Clear search between modes — the query meaning changes (video names
    // vs. grouping names).
    uiState.search = '';
    searchInput.value = '';
    searchClear.hidden = true;
...
refreshBtn.addEventListener('click', () => {
  ...
  refreshBtn.classList.add('header__view--spinning');
  rerenderCurrent().finally(() => {
    setTimeout(() => refreshBtn.classList.remove('header__view--spinning'), 400);
  });
});
...
function syncControlsUI() {
  ...
  for (const btn of modeTabsEl.querySelectorAll('.controls__mode')) {
    btn.classList.toggle('controls__mode--active', btn.dataset.mode === currentMode);
  }
  // Tab active state
  for (const tab of document.querySelectorAll('.controls__tab')) {
    tab.classList.toggle('controls__tab--active', tab.dataset.tab === uiState.tab);
  }
  // Sort + VR
  sortSelect.value = uiState.sort;
  vrSelect.value = uiState.vr;
  // Search input
  ...
  viewToggle.replaceChildren(svgIcon(uiState.view === 'grid' ? 'layoutList' : 'layoutGrid', 16));
